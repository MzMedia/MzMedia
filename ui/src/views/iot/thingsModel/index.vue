<template>
  <div class="p-2">
      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="是否历史存储" prop="isHistory">
              <el-input v-model="queryParams.isHistory" placeholder="请输入是否历史存储" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['iot:thingsModel:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['iot:thingsModel:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['iot:thingsModel:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['iot:thingsModel:export']">导出</el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" :data="thingsModelList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="是否历史存储" align="center" prop="isHistory" />
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['iot:thingsModel:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['iot:thingsModel:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="thingsModelFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="物模型名称" prop="modelName">
        ...
        <el-form-item label="是否历史存储" prop="isHistory">
          <el-input v-model="form.isHistory" placeholder="请输入是否历史存储" />
        </el-form-item>
        <el-form-item label="是否只读数据(0-否，1-是)" prop="isReadonly">
...
<script setup name="ThingsModel" lang="ts">
import { listThingsModel, getThingsModel, delThingsModel, addThingsModel, updateThingsModel } from '@/api/iot/thingsModel';
import { ThingsModelVO, ThingsModelQuery, ThingsModelForm } from '@/api/iot/thingsModel/types';
...
const data = reactive<PageData<ThingsModelForm, ThingsModelQuery>>({
  ...
  rules: {
    ...
    isHistory: [
      { required: true, message: "是否历史存储不能为空", trigger: "blur" }
    ],
    isReadonly: [
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: ThingsModelVO[]) => {
...
const handleUpdate = async (row?: ThingsModelVO) => {
  ...
  const res = await getThingsModel(_id);
...
const handleDelete = async (row?: ThingsModelVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除物模型编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await delThingsModel(_ids);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('iot/thingsModel/export', {
    ...queryParams.value
  }, `thingsModel_${new Date().getTime()}.xlsx`)
}
